...
<tr>
    ...
    <td>
        ...
        @for($i = 0; $i <= 100; $i += 5) {{-- step by 10, can change to 1 if needed --}}
...
$(document).on('click', '.btn-success', function () {
  //alert("Dfsffds");
    ...
    let projectId = $(this).val();     // button value = project id

    // collect fields from row
    let data = {
        id: projectId,
        _token: '{{ csrf_token() }}',
        fieldValues: {}
    ...
    // get all dropdowns and date inputs inside row
    row.find('select, input[type="date"]').each(function() {
        let field = $(this).data('field');   // field name from data-field
        let value = $(this).val();
        if (field) {
            data.fieldValues[field] = value;
        }
    });

    // send Ajax for each field
    $.each(data.fieldValues, function(field, value) {
        $.ajax({
            url: "{{ route('projects.updateField') }}",
            type: "POST",
            data: {
                id: projectId,
                field: field,
